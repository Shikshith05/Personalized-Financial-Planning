...
class SimpleGoalEnv:
    def __init__(
        ...
        # Store real user savings for every month
        self.monthly_savings_history = []

        # Previous month's recommendation
        self.previous_recommended_savings = 0.0

        # Previous month's amount actually saved by the user
        self.previous_actual_savings = 0.0
    ...
    def step(self, action, actual_savings=None):
        ...
        # During training there is no real user, so the user is
        # assumed to follow the recommendation exactly. During
        # real monthly usage, the caller passes what the user
        # actually saved.
        if actual_savings is None:
            actual_savings = recommended_savings
        else:
            actual_savings = float(actual_savings)

        actual_savings = max(
            0.0,
            min(actual_savings, available_amount),
        )

        self.monthly_savings_history.append(
            actual_savings
        )

        self.previous_recommended_savings = (
            recommended_savings
        )

        self.previous_actual_savings = (
            actual_savings
        )
        ...
        # Penalize recommendations the user does not follow, so
        # the model learns to suggest achievable amounts.
        savings_gap = (
            recommended_savings
            - actual_savings
        )
    ...
    def _get_obs(self):
        ...
        previous_actual_savings_ratio = (
            self.previous_actual_savings
            / available_amount
        )

        if self.monthly_savings_history:
            average_savings = (
                sum(self.monthly_savings_history)
                / len(self.monthly_savings_history)
            )
        else:
            average_savings = 0.0
        ...
        recommendation_gap_ratio = (
            self.previous_recommended_savings
            - self.previous_actual_savings
        ) / available_amount

        previous_actual_savings_ratio = np.clip(
            previous_actual_savings_ratio,
            0.0,
            1.5,
        )

        average_savings_ratio = np.clip(
            average_savings_ratio,
            0.0,
            1.5,
        )

        recommendation_gap_ratio = np.clip(
            recommendation_gap_ratio,
            -1.5,
            1.5,
        )
