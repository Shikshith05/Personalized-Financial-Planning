...
def train_model(
    ...
    for episode in range(1, episodes + 1):
        ...
        # Each simulated user keeps one behaviour type for the
        # whole episode; monthly compliance varies within it.
        behaviour_type = np.random.choice(
            [
                "low_saver",
                "inconsistent_saver",
                "good_saver",
        ...
        while not done:
            ...
            recommended_savings = (
                recommended_rate * available_amount
            )

            if behaviour_type == "low_saver":
                compliance_ratio = np.random.uniform(
                    0.1,
                    0.5,
                )

            elif behaviour_type == "inconsistent_saver":
                compliance_ratio = np.random.uniform(
                    0.3,
                    1.0,
                )
...
def evaluate_model(env, model):
    ...
    with torch.no_grad():
        while not done:
            ...
            print(
                f"Savings amount: "
                f"Rs.{info['recommended_savings']:.2f}"
            )
